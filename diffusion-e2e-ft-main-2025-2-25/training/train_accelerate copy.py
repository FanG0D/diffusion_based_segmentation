...
def main():
    ...
    optimizer = optimizer_cls(
        list(unet.parameters()) + list(logits_decoder.parameters()), 
    ...
    # Training datasets
    ade20k_root_dir = "/root/autodl-tmp/train_dataset/ade20k"
    train_dataset_ade20k = ADE20K(root_dir=ade20k_root_dir, transform=True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):     
                ...
                seg_latents = encode_image(vae, batch["seg"].to(device=accelerator.device, dtype=weight_dtype))
                seg_latents = seg_latents * vae.config.scaling_factor
                ...
                ground_truth = batch["annotation"].to(device=accelerator.device, dtype=weight_dtype)

                # Compute task-specific loss   
                estimation_loss = 0
                estimation_loss_ce = ce_loss(logits_pred, ground_truth)
    ...
    if accelerator.is_main_process:
        ...
        logits_decoder = unwrap_model(logits_decoder)
