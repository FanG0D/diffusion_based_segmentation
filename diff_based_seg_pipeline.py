...
class DiffBasedSegPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # 预测maskiage对应的图
        duplicated_rgb = rgb.expand(ensemble_size, -1, -1, -1)
        ...
        maskiage_preds = torch.concat(maskiage_pred_ls, dim=0)
    ...
    generator: Union[torch.Generator, None],
        ...
        seg = self.decode_seg(seg_latent)


        # 分割任务的输出结果是几个类别，所以需要进行one-hot编码所以不能简单的clip
