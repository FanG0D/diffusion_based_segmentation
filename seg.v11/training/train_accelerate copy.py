...
def main():
    ...
    if args.noise_type is not None:
        ...
        if unet.config['in_channels'] != 8:
            replace_unet_conv_in(unet, repeat=3)
    ...
    # Training datasets
    ade20k_train_root_dir = "/mnt/diff_based_seg/train_dataset/ade20k/ade20k_train"
    ...
    # Validation datasets
    ade20k_val_root_dir = "/mnt/diff_based_seg/train_dataset/ade20k/ade20k_val"
    ...
    ade20k_vis_root_dir = "/mnt/diff_based_seg/train_dataset/ade20k/ade20k_vis"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):     
                ...
                unet_input = (
                    torch.cat((rgb_latents, depth_latents, noisy_latents), dim=1).to(accelerator.device)
                    if args.noise_type is not None
                    else rgb_latents
                )   
                ...
                estimation_loss = 0
                total_mse_loss = mse_loss(model_pred.float(), seg_latents.float())
...
def validation_step(args, accelerator, model_components, val_dataloader):
    ...
    for step, batch in val_progress:
        with torch.no_grad():
            ...
            # depth latent
            depth_latents = encode_image(vae, batch["depth"].to(device=accelerator.device))
            depth_latents = depth_latents * vae.config.scaling_factor
            ...
            # Generate UNet prediction 
            encoder_hidden_states = empty_encoding.repeat(len(batch["rgb"]), 1, 1)
            unet_input = torch.cat((rgb_latents, depth_latents, latent), dim=1)
...
def visualization_step(args, accelerator, model_components, vis_dataloader, step):
    ...
    for idx, batch in enumerate(vis_dataloader):
        with torch.no_grad():
            ...
            current_estimate = (current_estimate + 1.0) / 2.0
            mask = decode_from_segmap(current_estimate * 255, keep_ignore_index=True, prob=False).squeeze(1)
            mask = current_estimate.squeeze().mean(dim=0).cpu().numpy()  
