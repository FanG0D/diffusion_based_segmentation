...
if "__main__" == __name__:
    ...
    parser = argparse.ArgumentParser(
        description="Run single-image segmentation estimation using diffbasedseg."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    if modality == 'seg':
        output_dir_color = os.path.join(output_dir, "seg_colored")
        output_dir_npy = os.path.join(output_dir, "seg_npy")
    ...
    with torch.no_grad():
        ...
        for rgb_path in tqdm(rgb_filename_list, desc="Estimating segmentation", leave=True):
            # Read input image
            ...
            # add
            pred: np.ndarray = pipe_out.mask
            pred_colored: Image.Image = pipe_out.maskiage
