...
class ADE20KDataset(BaseSegDataset):
    ...
    def _read_seg_file(self, rel_path):
        ...
        annotation_in = Image.fromarray(annotation_in.astype(np.uint8))
        seg_in_origin = annotation_in
        seg_in_origin = np.asarray(seg_in_origin) # [H, W]
        ...
        return seg_in_rgb, seg_in_origin

    def _load_seg_data(self, seg_rel_path):
        maskiage, seg_raw_origin = self._read_seg_file(seg_rel_path)
        outputs = {
            "annotation_raw": torch.from_numpy(seg_raw_origin).int(),
            "maskiage": torch.from_numpy(maskiage).int(),
        }
