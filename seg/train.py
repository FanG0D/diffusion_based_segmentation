...
if "__main__" == __name__:
    ...
    trainer_cls = get_trainer_cls(cfg.trainer.name) # 修改get_trainer_cls
    ...
    # -------------------- Checkpoint --------------------
    if resume_run is not None:
        trainer.load_checkpoint( # 修改load_checkpoint 在marigold_trainer.py中
            resume_run, load_trainer_state=True, resume_lr_scheduler=True
        )

    # -------------------- Training & Evaluation Loop --------------------
    try:
        trainer.train(t_end=t_end) # 修改train 在marigold_trainer.py中
