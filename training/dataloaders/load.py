...
class SynchronizedTransform_ADE20K:
    def __init__(self, H, W, crop_size=(300, 400)):
        self.H = H
        self.W = W
    ...
    def random_crop(self, rgb_image, seg_img):
        ...
        if w < tw or h < th:
            ...
            seg_img = seg_img.resize((self.W, self.H), Image.NEAREST)
            w, h = rgb_image.size
    ...
    def __call__(self, rgb_image, seg_img):
        ...
        rgb_image = self.resize(rgb_image)
        seg_in_rgb = np.array([
            cv2.resize(seg_in_rgb[i], (self.W, self.H), 
        ...
        resized_one_hot = np.array([
            cv2.resize(one_hot[c], (self.W, self.H), 
                    interpolation=cv2.INTER_LINEAR)
            for c in range(num_classes)
        ])
        seg_in_origin = np.argmax(resized_one_hot, axis=0)

        # seg_in_origin = cv2.resize(seg_in_origin, (self.W, self.H), 
...
class ADE20K(Dataset):
    ...
    def _find_pairs(self):
        # root_dir/
        #   ©À©¤©¤ ade20k_train/images    # RGB images
