...
def main():
    ...
    # logits_decoder = torch.nn.Conv2d(3, 151, kernel_size=1)
    ...
    # unet, logits_decoder, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
    #     unet, logits_decoder, optimizer, train_dataloader_ade20k, lr_scheduler
    # )
    unet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
    unet, optimizer, train_dataloader_ade20k, lr_scheduler)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):     
                ...
                # seg latent
                # ground_truth = batch["seg"].to(device=accelerator.device, dtype=weight_dtype)
                # seg_latents = encode_seg(vae, ground_truth, accelerator.device)
                # seg_latents = seg_latents * vae.config.scaling_factor
                ...
                current_estimate = (current_estimate + 1.0) / 2.0   # B, 3, H, W
                # current_estimate = current_estimate * 255  
                color_map = ade_palette()
                normalized_color_map = torch.tensor(color_map) / 255.0
                B, _, H, W = current_estimate.shape
                C = normalized_color_map.shape[0]
                sigma = 10.0
                current_estimate = current_estimate.permute(0, 2, 3, 1).float() # B, H, W, 3
                class_colors = normalized_color_map.unsqueeze(0).unsqueeze(0).unsqueeze(0) # 1, 1, 1, C, 3
                class_colors = class_colors.float().to(current_estimate.device) # B, H, W, C, 3
                distances = torch.sqrt(torch.sum((current_estimate.unsqueeze(3) - class_colors) ** 2, dim=4))  # B, H, W, C
                similarities = torch.exp(-distances ** 2 / (2 * sigma ** 2))  # B, H, W, C 
                pseudo_logits = torch.log_softmax(similarities, dim=3)  # B, H, W, C
                pseudo_logits = pseudo_logits.permute(0, 3, 1, 2) # B, C, H, W

                estimation_loss = 0
                estimation_loss_ce = ce_loss(pseudo_logits, batch["annotation"].to(device=accelerator.device, dtype=weight_dtype))
                estimation_loss_mse = mse_loss(current_latent_estimate, seg_latents_origin.float())
                if not torch.isnan(estimation_loss_mse).any():
                    estimation_loss = estimation_loss + mse_loss_weight * estimation_loss_mse

                if not torch.isnan(estimation_loss_ce).any():
                    estimation_loss = estimation_loss + ce_loss_weight * estimation_loss_ce
...
def validation_step(args, accelerator, model_components, val_dataloader):
    ...
    for step, batch in val_progress:
        with torch.no_grad():
            ...
            del rgb_latents, latent, noise_pred
